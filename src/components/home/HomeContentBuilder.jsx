...
const HomeContentBuilder = () => {
  ...
  // Map to Mongoose Schema property 'sections' with fallback to 'homeSections' or default array
  const rawSections =
  ...
  // Filter out any disabled sections
  const sections = rawSections.filter((sec) => sec.enabled !== false);

  // Filter and map FAQs
  const rawFaqs = Array.isArray(home?.faqs) ? home.faqs : [];
  const faqs = rawFaqs.filter((faq) => faq.enabled !== false);

  const renderSection = (section, index) => {
    const sectionType = section.sectionType || section.type || "paragraph";
    ...
    if (sectionType === "heading") {
      return (
        <div
          ...
          className={`mt-8 first:mt-0 ${isSplit ? "md:col-span-2" : ""}`}
        >
          <div className="rounded-[24px] bg-primary/5 px-6 py-8 md:px-8 md:py-10">
            ...
            {section.subtitle && (
              <p className="mt-2 text-lg font-medium text-textGray">
                {section.subtitle}
              </p>
            )}
          </div>
          ...
          <div className={shellClass}>
            ...
            <ul className="mt-4 grid gap-3 md:grid-cols-2">
              {(section.items || []).map((item, itemIndex) => (
                <li
                  key={`${section._id || index}-${itemIndex}`}
                  className="flex gap-3 text-base leading-7 text-textGray"
                >
    ...
    if (sectionType === "imageBanner" && section.image?.url) {
      return (
        <div
          key={section._id || index}
          className={`mt-8 first:mt-0 ${isSplit ? "md:col-span-2" : ""}`}
        >
          <div className="overflow-hidden rounded-[24px] bg-lightBg">
            <img
            ...
            {(section.title || section.content) && (
              <div className="p-6">
                ...
                {section.content && (
                  <p className="mt-2 text-base text-textGray">{section.content}</p>
                )}
              </div>
            )}
          </div>
    ...
    <section className="w-full bg-white py-10 md:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {sections.length > 0 ? (
          <div
            className={
              sections.some((section) => section.layout === "split")
          ...
          <div className="rounded-[24px] bg-lightBg p-8 text-center text-textGray">
            Add custom home sections from the admin panel to display content here.
          </div>
        )}
              ...
              {faqs.map((faq, index) => {
                ...
                return (
                  <div
                    ...
                    className="rounded-[20px] bg-lightBg/70 px-5 py-5"
                  >
                    <button
                      type="button"
                      onClick={() =>
